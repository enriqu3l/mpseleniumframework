...
public class HomeVuelosPage {
	...
	private static Logger logger = LogManager.getLogger(HomeVuelosPage.class);
	
	//Constructor
	public HomeVuelosPage(WebDriver _driver){
		...
		wait.until(ExpectedConditions.urlContains("/vuelos"));
		logger.trace("--->CurrentURL:"+driver.getCurrentUrl());
	}
	
	public void goTo() {
		Pages.topnav(driver).clickVuelos();
	}
