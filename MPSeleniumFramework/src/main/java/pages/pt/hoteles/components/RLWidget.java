...
public class RLWidget extends HLGlobal {
	...
	private static Logger logger = LogManager.getLogger(RLWidget.class);

	public RLWidget(WebDriver _driver) {
		super(_driver);
		this.driver = _driver;
	...
	public void setOccupants(VOResData voHotelRes) {
		...
		if (widgetAllBlockRooms.size() != voHotelRes.getRoomCount()) {
			logger.error(
					"No se crearon los suficientes rooms containers, allBlocksRooms:" + widgetAllBlockRooms.size());
			Assert.fail("LAF>>>No se crearon los suficientes rooms containers, allBlocksRooms:"
					+ widgetAllBlockRooms.size());
		}
		...
		if (widgetMinorsAgesHotelContainer.size() != voHotelRes.getRoomCount()) {
			...
			logger.error(
					"La cantidad de rooms no coincide con la cantidad de agecontainers, widgetMinorsAgesHotelContainer:"
							+ widgetMinorsAgesHotelContainer.size());
			Assert.fail(
					"LAF>>>La cantidad de rooms no coincide con la cantidad de agecontainers, widgetMinorsAgesHotelContainer:"
							+ widgetMinorsAgesHotelContainer.size());
		}
	...
	public void setEndDate(String date) {
		...
		// Abrir el calendario si no esta abierto
		if (FWUtils.noExistsElement(driver, byWidgetEndDateDropdownMenu)) {
	...
	public boolean verifyAutocompleteDestination(List<String> words) {
		...
		if (words.isEmpty()) {
			Assert.fail("LAF>>>El parametro words esta vacio");
		}
		boolean result = true;
	...
	public boolean verifyOccupantsToBe(VOResData voHotelRes) {
		...
		if (voHotelRes.getKidsCount() > 0) {
			...
			if (widgetMinorsAgesHotelContainer.size() != voHotelRes.getRoomCount()) {
				logger.error("No se crearon los containers de los agekids, widgetAllBlockMinorsAges:"
						+ widgetMinorsAgesHotelContainer.size());
				return false;
			}
			...
			for (int i = 0; i < widgetMinorsAgesHotelContainer.size(); i++) {
				...
				if (voHotelRes.getKidsFromRoom(i) > 0) {
					// Entra solo si el cuarto contiene niños
					List<WebElement> minors = widgetMinorsAgesHotelContainer.get(i)
							.findElements(byWidgetAllKidsPerRoom);
					for (int j = 0; j < minors.size(); j++) {
						// Recorrer cada niño y setear la edad
						Select minor = new Select(minors.get(j));
						...
						if (!Integer.toString(voHotelRes.getAgeFromAgekids(i, j)).equalsIgnoreCase(ageMinorSelected)) {
							logger.error("No coincide Edad del Niño:" + j + ", en el room:" + i + ", ageMinorSelected:"
									+ ageMinorSelected + ".");
							result = false;
							break;
						}
			...
			if (widgetMinorsAgesHotelContainer.size() > 0) {
				logger.error("Se encontraron age containers y no deberia haber, widgetAllBlockMinorsAges:"
						+ widgetMinorsAgesHotelContainer.size());
				result = false;
			}
		}
	...
	public boolean verifyReservationToBe(VOResData voHotelRes) {
		...
		return getDestination().equalsIgnoreCase(voHotelRes.getDestination())
				&& getStartDate().equals(voHotelRes.getStartDate()) && getEndDate().equals(voHotelRes.getEndDate())
	...
	/**
	 * Esta funcion verifica que se abran y cierren los dropdowns de las fechas en
	 * 10 iteraciones
	 */
