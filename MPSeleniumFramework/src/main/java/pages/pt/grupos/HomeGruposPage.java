...
public class HomeGruposPage {
	private WebDriverWait wait;
	private WebDriver driver;
	private static Logger logger = LogManager.getLogger(HomeGruposPage.class);
	
	//Constructor
	public HomeGruposPage(WebDriver _driver){
		...
		wait.until(ExpectedConditions.urlContains("/grupos/informacion-grupo"));
		logger.trace("--->CurrentURL:"+driver.getCurrentUrl());
	}
	
	public void goTo() {
		Pages.topnav(driver).clickGrupos();
	}
	
	public boolean isAt() {
		//Verificar que estoy en esta pagina
		return driver.getTitle().contains("Cotizar un viaje en grupo");
	}
